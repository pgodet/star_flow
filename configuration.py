...
class CheckpointSaver:
    ...
    def restore(self, filename, model_and_loss, device, checkpoint_loading_strict, include_params="*", exclude_params=()):
        # -----------------------------------------------------------------------------------------
        ...
        self._load_state_dict_into_module(state_dict, model_and_loss, strict=checkpoint_loading_strict)
    ...
    def restore_latest(self, directory, model_and_loss, device, checkpoint_loading_strict, include_params="*", exclude_params=()):
        latest_checkpoint_filename = os.path.join(
            directory, self._prefix + self._latest_postfix + self._extension)
        return self.restore(latest_checkpoint_filename, model_and_loss, device,
                            checkpoint_loading_strict, include_params, exclude_params)

    def restore_best(self, directory, model_and_loss, device, checkpoint_loading_strict, include_params="*", exclude_params=()):
        best_checkpoint_filename = os.path.join(
            directory, self._prefix + self._best_postfix + self._extension)
        return self.restore(best_checkpoint_filename, model_and_loss, device,
                            checkpoint_loading_strict, include_params, exclude_params)
    ...
    with logger.LoggingBlock("Checkpoint", emph=True):
        ...
        if args.checkpoint is None:
            ...
            checkpoint_stats, filename = checkpoint_saver.restore(
                ...
                device=args.device,
                checkpoint_loading_strict=args.checkpoint_loading_strict,
...
def configure_data_loaders(args):
    with logger.LoggingBlock("Datasets", emph=True):
        ...
        def _log_statistics(dataset, prefix, name):
            with logger.LoggingBlock("%s Dataset: %s" % (prefix, name)):
                ...
                for key, value in sorted(example_dict.items()):
                    if key in ["index", "basename", "basedir", "nframes", "idx_first"]:  # no need to display these
                        continue
                    if isinstance(value, str):
                        logging.info("{}: {}".format(key, value))
                    elif isinstance(value, list) or isinstance(value, tuple):
                        logging.info("%s: %s" % (key, _sizes_to_str(value[0])))
                    else:
        ...
        if args.inference_dataset is not None:
            ...
            # ----------------------------------------------
            # Figure out inference_dataset arguments
            # ----------------------------------------------
            kwargs = tools.kwargs_from_args(args, "inference_dataset")
            kwargs["is_cropped"] = True
            kwargs["args"] = args
            ...
            # ----------------------------------------------
            inference_dataset = tools.instance_from_kwargs(args.inference_dataset_class, kwargs)
            ...
            _log_statistics(inference_dataset, prefix="Inference", name=args.inference_dataset)
    ...
    with logger.LoggingBlock("Optimizer", emph=True):
        if args.optimizer is not None:
            if model_and_loss.num_parameters() == 0:
                ...
                if args.optim_checkpoint:
                    checkpoint_with_state = torch.load(args.optim_checkpoint,
                                map_location=lambda storage,
                                loc: storage.cuda(args.device))

                    if 'optim_state_dict' in checkpoint_with_state.keys():
                        state_dict = checkpoint_with_state['optim_state_dict']
                    elif 'state_dict' in checkpoint_with_state.keys():
                        state_dict = checkpoint_with_state['state_dict']
                    else:
                        state_dict = checkpoint_with_state

                    optimizer.load_state_dict(state_dict)
                    logging.info("Optim state dict loaded : %s." % args.optim_checkpoint)
