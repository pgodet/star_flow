...
def main():
    ...
    if args.resume:
        args.checkpoint = os.path.join(args.save, 'checkpoint_latest.ckpt')
        args.optim_checkpoint = os.path.join(args.save, 'optim_state_dict_checkpoint.pth')
        checkpoint_with_state = torch.load(args.checkpoint,
                                 map_location=lambda storage,
                                 loc: storage.cuda(args.device))
        args.start_epoch = checkpoint_with_state['epoch'] + 1
