...
class TRFeat(nn.Module):
    def __init__(self, args, div_flow=0.05):
        super(TRFeat, self).__init__()
        self.args = args
        ...
        self.conv_1x1_time = nn.ModuleList([
                    conv(2 * self.dim_corr + 448, self.dim_corr, kernel_size=1, stride=1, dilation=1),
                    conv(2 * (self.dim_corr + self.num_chs[-2] + 2) + 448, self.dim_corr + self.num_chs[-2] + 2, kernel_size=1, stride=1, dilation=1),
                    conv(2 * (self.dim_corr + self.num_chs[-3] + 2) + 448, self.dim_corr + self.num_chs[-3] + 2, kernel_size=1, stride=1, dilation=1),
                    conv(2 * (self.dim_corr + self.num_chs[-4] + 2) + 448, self.dim_corr + self.num_chs[-4] + 2, kernel_size=1, stride=1, dilation=1),
                    conv(2 * (self.dim_corr + self.num_chs[-5] + 2) + 448, self.dim_corr + self.num_chs[-5] + 2, kernel_size=1, stride=1, dilation=1)])
        ...
        for i in range(len(list_imgs) - 1):
            ...
            for l, (x1, x2) in enumerate(zip(x1_pyramid, x2_pyramid)):
                if i == 0:
                    ...
                    if l == 0:
                        previous_features.append(torch.zeros(bs_, self.dim_corr, h_, w_, dtype=init_dtype, device=init_device).float())
                    else:
                        previous_features.append(torch.zeros(bs_, self.dim_corr + self.num_chs[-1-l] + 2, h_, w_, dtype=init_dtype, device=init_device).float())
                ...
                if l == 0:
                    ...
                    features = torch.cat([previous_features[l], out_corr_relu], 1)
                    features_b = torch.cat([torch.zeros_like(previous_features[l]), out_corr_relu_b], 1)
                    x_intm, flow = self.flow_estimators[l](features)
                    with torch.no_grad():
                        x_intm_b, flow_b = self.flow_estimators[l](features_b)
                    previous_features[l] = self.conv_1x1_time[l](x_intm)
                else:
                    if i > 0: #temporal connection:
                        previous_features[l] = self.warping_layer(previous_features[l],
                                        flows_b[l][-1], height_im, width_im, self._div_flow)
                    features = torch.cat([previous_features[l], out_corr_relu, x1, flow], 1)
                    features_b = torch.cat([torch.zeros_like(previous_features[l]), out_corr_relu_b, x2, flow_b], 1)
                    x_intm, flow = self.flow_estimators[l](features)
                    with torch.no_grad():
                        x_intm_b, flow_b = self.flow_estimators[l](features_b)
                    previous_features[l] = self.conv_1x1_time[l](x_intm)
...
class TRFeat_occjoint(nn.Module):
    def __init__(self, args, div_flow=0.05):
        super(TRFeat_occjoint, self).__init__()
        self.args = args
        self._div_flow = div_flow
        self.search_range = 4
        self.num_chs = [3, 16, 32, 64, 96, 128, 196]
        ...
        self.conv_1x1_time = nn.ModuleList([
                    conv(2 * self.dim_corr + 448, self.dim_corr, kernel_size=1, stride=1, dilation=1),
                    conv(2 * (self.dim_corr + self.num_chs[-2] + 2 + 1) + 448, self.dim_corr + self.num_chs[-2] + 2 + 1, kernel_size=1, stride=1, dilation=1),
                    conv(2 * (self.dim_corr + self.num_chs[-3] + 2 + 1) + 448, self.dim_corr + self.num_chs[-3] + 2 + 1, kernel_size=1, stride=1, dilation=1),
                    conv(2 * (self.dim_corr + self.num_chs[-4] + 2 + 1) + 448, self.dim_corr + self.num_chs[-4] + 2 + 1, kernel_size=1, stride=1, dilation=1),
                    conv(2 * (self.dim_corr + self.num_chs[-5] + 2 + 1) + 448, self.dim_corr + self.num_chs[-5] + 2 + 1, kernel_size=1, stride=1, dilation=1)])
            ...
            for l, (x1, x2) in enumerate(zip(x1_pyramid, x2_pyramid)):
                if i == 0:
                    ...
                    if l == 0:
                        ...
                        previous_features.append(torch.zeros(bs_, self.dim_corr + self.num_chs[-1-l] + 2 + 1, h_, w_, dtype=init_dtype, device=init_device).float())
                ...
                if l == 0:
                    ...
                    if i > 0: #temporal connection:
                        previous_features[l] = self.warping_layer(previous_features[l],
                                        flows_b[l][-1], height_im, width_im, self._div_flow)
                    features = torch.cat([previous_features[l], out_corr_relu, x1, flow, occ], 1)
                    features_b = torch.cat([torch.zeros_like(previous_features[l]), out_corr_relu_b, x2, flow_b, occ_b], 1)
                    x_intm, flow, occ = self.flow_and_occ_estimators[l](features)
                    with torch.no_grad():
                        x_intm_b, flow_b, occ_b = self.flow_and_occ_estimators[l](features_b)
                    previous_features[l] = self.conv_1x1_time[l](x_intm)
    ...
    def forward(self, input_dict):
        ...
        for i in range(len(list_imgs) - 1):
            ...
            for l, (x1, x2) in enumerate(zip(x1_pyramid, x2_pyramid)):
                ...
                features = torch.cat([previous_features[l], out_corr_relu, x1_1by1, flow, occ], 1)
                features_b = torch.cat([torch.zeros_like(previous_features[l]), out_corr_relu_b, x2_1by1, flow_b, occ_b], 1)

                x_intm, flow_res, occ_res = self.flow_and_occ_estimators(features)
                flow = flow + flow_res
                occ = occ + occ_res
                with torch.no_grad():
                    x_intm_b, flow_b_res, occ_b_res = self.flow_and_occ_estimators(features_b)
                ...
                previous_features[l] = self.conv_1x1_time(x_intm)
                flows[l].append(flow)
                occs[l].append(occ)
                flows_b[l].append(flow_b)
