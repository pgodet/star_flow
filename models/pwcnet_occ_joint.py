...
class PWCNet(nn.Module):
    def __init__(self, args, div_flow=0.05):
        ...
        for l, ch in enumerate(self.num_chs[::-1]):
            ...
            if l == 0:
                ...
                num_ch_in = self.dim_corr + ch + 2 + 1

            layer = FlowAndOccEstimatorDense(num_ch_in)
            self.flow_and_occ_estimators.append(layer)

        self.context_networks = FlowAndOccContextNetwork(self.dim_corr + 32 + 2 + 1 + 448 + 2 + 1)
    ...
    def forward(self, input_dict):
        ...
        for l, (x1, x2) in enumerate(zip(x1_pyramid, x2_pyramid)):
            ...
            # flow estimator
            if l == 0:
                x_intm, flow, occ = self.flow_and_occ_estimators[l](out_corr_relu)
            else:
                x_intm, flow, occ = self.flow_and_occ_estimators[l](torch.cat([out_corr_relu, x1, flow, occ], dim=1))
