            ...
            for key, value in example_dict.items():
                if key in tensor_keys:
                    if isinstance(value, list) or isinstance(value, tuple):
                        example_dict[key] = [value_i.cuda(non_blocking=False) for value_i in value]
                    else:
        ...
        for key, tensor in example_dict.items():
            if key in input_keys:
                if isinstance(tensor, list) or isinstance(tensor, tuple):
                    example_dict[key] = [tensor_i.requires_grad_(True) for tensor_i in tensor]
                else:
                    example_dict[key] = tensor.requires_grad_(True)
            elif key in target_keys:
                if isinstance(tensor, list) or isinstance(tensor, tuple):
                    example_dict[key] = [tensor_i.requires_grad_(False) for tensor_i in tensor]
                else:
...
class EvaluationEpoch:
    ...
    def save_outputs(self, example_dict, output_dict):
        ...
        save_root_occ = self._args.save + '/occ/'
        ...
        for ii in range(0, b_size):
            ...
            directory_occ = os.path.dirname(file_name_occ)
            if not os.path.exists(directory_occ):
                os.makedirs(directory_occ)

        if self._args.save_result_img or self._args.save_result_occ:
            for ii in range(0, b_size):
                if self._args.save_result_occ:
                    file_name_occ = file_names_occ[ii] + '_occ.png'
                    scipy.misc.imsave(file_name_occ, output_occ[ii])

                    if self._args.save_result_bidirection:
                        scipy.misc.imsave(file_names_occ[ii] + '_occ_b.png', output_occ_b[ii])
                ...
                if self._args.save_result_img and self._args.save_result_bidirection:
    ...
    def save_outputs_multiframe(self, example_dict, output_dict):
        ...
        flow_f = output_dict["flow"].data.cpu().numpy()
        nframes = output_dict["flow"].data.size(0)
        b_size = output_dict["flow"].data.size(1)

        if self._args.save_result_occ:
            output_occ = np.round(
                        nn.Sigmoid()(output_dict["occ"]).expand(
                            -1, -1, 3, -1, -1).data.cpu().numpy().transpose(
                                [0, 1, 3, 4, 2])) * 255
        ...
        file_names_flo = []
        file_names_occ = []
        for tt in range(0, nframes):
            for ii in range(0, b_size):
                if "basedir" in  example_dict.keys():
                    file_name_img = save_root_img + example_dict["basedir"][ii] + '/' + str(example_dict["basename"][tt][ii])
                    file_name_flo = save_root_flo + example_dict["basedir"][ii] + '/' + str(example_dict["basename"][tt][ii])
                    file_name_occ = save_root_occ + example_dict["basedir"][ii] + '/' + str(example_dict["basename"][tt][ii])
                else:
                    file_name_img = save_root_img + '/' + str(example_dict["basename"][tt][ii])
                    file_name_flo = save_root_flo + '/' + str(example_dict["basename"][tt][ii])
                    file_name_occ = save_root_occ + '/' + str(example_dict["basename"][tt][ii])
                ...
                if self._args.save_result_img or self._args.save_result_occ:
                    if self._args.save_result_occ:
                        j = 1
                        while True:
                            file_name = file_name_occ + '_occ_{}.png'.format(j)
                            if os.path.exists(file_name):
                                j += 1
                            else:
                                break
                        #file_name = file_name_img + '_occ.png'
                        scipy.misc.imsave(file_name, output_occ[tt, ii])


                    # flow vis
                    if self._args.save_result_img:
                        flow_f_rgb = flow_to_png_middlebury(flow_f[tt, ii, ...])
                        j = 1
                        while True:
                            file_name_flo_vis = file_name_img + '_flow_{}.png'.format(j)
                            if os.path.exists(file_name_flo_vis):
                                j += 1
                            else:
                                break
                        scipy.misc.imsave(file_name_flo_vis, flow_f_rgb)
                ...
                if self._args.save_result_flo or self._args.save_result_png:
                    if self._args.save_result_flo:
                        j = 1
                        while True:
                            file_name = file_name_flo + '_{}.flo'.format(j)
                            if os.path.exists(file_name):
                                j += 1
                            else:
                                break
                        write_flow(file_name, flow_f[tt, ii, ...].swapaxes(0, 1).swapaxes(1, 2))
                    if self._args.save_result_png:
                        j = 1
                        while True:
                            file_name = file_name_flo + '_{}.png'.format(j)
                            if os.path.exists(file_name):
                                j += 1
                            else:
                                break
                        write_flow_png(file_name, flow_f[tt, ii, ...].swapaxes(0, 1).swapaxes(1, 2))
            ...
            with create_progressbar(**progressbar_args) as progress:
                for example_dict in progress:
                    ...
                    if self._save_output:
                        if output_dict['flow'].data.ndimension() == 4:
                            self.save_outputs(example_dict, output_dict)
                        else:
                            self.save_outputs_multiframe(example_dict, output_dict)
...
class InferenceEpoch(EvaluationEpoch):
    def __init__(self,
                 args,
                 model,
                 loader,
    ...
    def _step(self, example_dict):
        ...
        # Run forward pass to get losses and outputs.
        # -------------------------------------------------------------
        #loss_dict, output_dict = self._model(example_dict)
...
def exec_runtime(args,
    ...
    # --------------------------------------------------------
    best_validation_loss = float("inf") if (args.validation_loss and args.validation_key_minimize) else -float("inf")
    store_as_best = False
    ...
    for epoch in range(args.start_epoch, args.total_epochs + 1):
        with logger.LoggingBlock("Epoch %i/%i" % (epoch, args.total_epochs), emph=True):
            ...
            if train_loader is not None:
                ...
                # save loss value in file:
                with open(os.path.join(args.save, "train_loss.csv") ,'a') as loss_file:
                    if epoch == args.start_epoch:
                        loss_file.write("epoch,loss\n")
                    loss_file.write(str(epoch) + ', ' + str(avg_loss_dict[args.training_key]) + '\n')
            ...
            if validation_loader is not None:
                ...
                # save validation error in file:
                with open(os.path.join(args.save, "err_val.csv") ,'a') as errval_file:
                    if epoch == args.start_epoch:
                        errval_file.write("epoch,err\n")
                    errval_file.write(str(epoch) + ',' + str(avg_loss_dict[args.validation_key]) + '\n')

                # ----------------------------------------------------------------
                # Evaluate whether this is the best validation_loss
                # ----------------------------------------------------------------
                validation_loss = avg_loss_dict[args.validation_key]
                if (args.validation_loss and args.validation_key_minimize):
            ...
            # ----------------------------------------------------------------
            if (validation_loader is not None) or (train_loader is not None):
            ...
            if checkpoint_saver is not None:
                ...
                torch.save(optimizer.state_dict(), os.path.join(args.save, "optim_state_dict_checkpoint.pth"))
