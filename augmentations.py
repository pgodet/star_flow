...
class RandomAffineFlow(nn.Module):
    def __init__(self, args, addnoise=True, backward=False):
    ...
    def forward(self, example_dict):
        ...
        if self.backward:
            flo = self.transform_flow(flo, theta2, theta1)
        else:
...
class RandomAffineFlowVideo(RandomAffineFlow):
    def __init__(self, args, addnoise=True, crop=None):
        super(RandomAffineFlowVideo, self).__init__(args)
        self._args = args
        self._interp2 = Interp2(clamp=False)
        self._flow_interp2 = Interp2(clamp=False)
        self._meshgrid = Meshgrid()
    ...
    def random_crop(self, imgs, flows):
        ...
        for k in range(len(flows)):
            flows[k] = flows[k][:, :, str_y:end_y, str_x:end_x]

        return imgs, flows

    def forward(self, example_dict):
        imgs = example_dict["input_images"]
        flows = example_dict["target_flows"]
        im1 = imgs[0]
        ...
        # random flip images
        theta1, theta0 = self._random_mirror(theta1, theta0)

        for k in range(len(imgs)):
            imgs[k] = self.transform_image(imgs[k], theta1)
        for k in range(len(flows)):
            flows[k] = self.transform_flow(flows[k], theta1, theta1)

        if self._addnoise:
            stddev = np.random.uniform(0.0, 0.04)
            for k in range(len(imgs)):
                self._noise1.resize_as_(imgs[k])
                self._noise1.normal_(std=stddev)
                imgs[k] += self._noise1
                imgs[k].clamp_(0.0, 1.0)

        if self._crop is not None:
            imgs, flows = self.random_crop(imgs, flows)

        # construct updated dictionaries
        example_dict["input_images"] = imgs
        example_dict["target_flows"] = flows
...
class RandomAffineFlowOcc(nn.Module):
    def __init__(self, args, addnoise=True, crop=None, backward=False):
    ...
    def forward(self, example_dict):
        ...
        im2 = self.transform_image(im2, theta2)
        if self.backward:
            flo_f = self.transform_flow(flo_f, theta2, theta1)
            flo_b = self.transform_flow(flo_b, theta1, theta2)
        else:
...
class RandomAffineFlowOccVideo(RandomAffineFlowOcc):

    def random_crop(self, imgs, flows, occs):
        ...
        for k in range(len(occs)):
            occs[k] = occs[k][:, :, str_y:end_y, str_x:end_x]
    ...
    def forward(self, example_dict):
        ...
        for k in range(len(flows)):
            flows[k] = self.transform_flow(flows[k], theta1, theta1)
        for k in range(len(occs)):
            occs[k] = self.transform_image(occs[k], theta1)
        ...
        if self._crop is not None:
            imgs, flows, occs = self.random_crop(imgs, flows, occs)

        # construct updated dictionaries
        example_dict["input_images"] = imgs
        example_dict["target_flows"] = flows
        example_dict["target_occs"] = occs

        return example_dict


class RandomAffineFlowVideoKitti(RandomAffineFlowOccVideo):

    def __init__(self, args, addnoise=True, crop=None, backward=False):
        super(RandomAffineFlowVideoKitti, self).__init__(args)
        self._args = args
        self._interp2 = Interp2(clamp=False)
    ...
    def random_crop(self, imgs, flo_f, valid_mask):
    ...
    def forward(self, example_dict):
        ...
        for k in range(len(imgs)):
            imgs[k] = self.transform_image(imgs[k], theta1)
        flo_f, valid_mask = self.transform_flow(flo_f, theta1, theta1, valid_mask)
        ...
        if self._crop is not None:
            imgs, flo_f, valid_mask = self.random_crop(imgs, flo_f, valid_mask)
...
class RandomAffineFlowKITTI(nn.Module):
    def __init__(self, args, addnoise=True, crop=None):
        super(RandomAffineFlowKITTI, self).__init__()
        self._args = args
        self._interp2 = Interp2(clamp=False)
        self._flow_interp2 = Interp2MaskBinary(clamp=False)
        self._meshgrid = Meshgrid()
